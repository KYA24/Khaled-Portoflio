async function renderHome() {
  ...
  try {
    ...
    const achievementImage = (item) => {
      const linkedProject = projects.find((project) => (item.relatedProjects || []).includes(project.id) && project.image);
      return {
        src: linkedProject?.image || item.image,
        alt: linkedProject?.imageAlt || item.imageAlt || item.title
      };
    };
      ...
      return `
        <a class="career-card career-card-link" href="career-path.html?id=${encodeURIComponent(path.id)}">
          ...
          <div class="career-mini-skills">
            ${pathSkills.map((skill) => `<span>${escapeHTML(t(skill.name))}</span>`).join("")}
          </div>
